...
#Vecindario 3: inserción entre todas las rutas
def insercion_entre_todas_rutas(data,solucion_actual):
    mejor_solucion = solucion_actual.copy()
    mejor_distancia = calcular_distancia_total(data,solucion_actual)
    for i in range(len(solucion_actual)):
...
def optimizar(data,niter,media,desviacion_estandar, n_iter,tiempo_limite):
    R=int(data[0,1]) 
    Q=float(data[0,2]) 
    Th=float(data[0,3]) 
    new_arr = data[1:] 
    ...
    while j <= 3:
        ...
        inicio_tiempo=time.perf_counter()
        if j == 1:
            nueva_solucion = insercion_entre_todas_rutas(data,mejor_solucion)
        elif j == 2:
